...
app.route('/users')
    ...
    .post(async (req, res) => {
    try {
        const { email, name, passward } = req.body;
...
app.route('/maps')
    ...
    .post(async (req, res) => {
    try {
        const { name, creatorEmail, config } = req.body;
...
app.get("/auth/kakao/callback", async (req, res) => {
    ...
    try {
        ...
        if (axios.isAxiosError(error)) {
            console.error("Axios error during Kakao Login:", error.response?.data || error.message);
            res.status(500).json({
